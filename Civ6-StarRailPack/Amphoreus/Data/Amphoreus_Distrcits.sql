...
CREATE TABLE IF NOT EXISTS NW_Amphoreus_Districts
(
    DistrictType         TEXT NOT NULL PRIMARY KEY, -- 特色区域
    ReplacesDistrictType TEXT NOT NULL -- 被替代区域
);
--======================================================================

-- 通用UD
CREATE TRIGGER IF NOT EXISTS NW_Districts_Auto_Insert
    AFTER INSERT
    ON NW_Amphoreus_Districts
BEGIN

    INSERT OR REPLACE INTO Types(Type, Kind)
    VALUES (NEW.DistrictType,'KIND_DISTRICT');

    INSERT OR REPLACE INTO DistrictReplaces(CivUniqueDistrictType, ReplacesDistrictType)
    VALUES (NEW.DistrictType,NEW.ReplacesDistrictType);
    ...
    INSERT INTO Districts(DistrictType, TraitType, Name, Description, PrereqTech, PrereqCivic, Coast, Cost,
                          ...
                          MaxPerPlayer)
    SELECT NEW.DistrictType,
           'TRAIT_' || NEW.DistrictType,
           'LOC_' || NEW.DistrictType || '_NAME',
           'LOC_' || NEW.DistrictType || '_DESCRIPTION',
           PrereqTech,
           PrereqCivic,
           Coast,
           Cost / 2,
           ...
           MaxPerPlayer                --最多数量
    FROM Districts WHERE DistrictType = NEW.ReplacesDistrictType;
    ...
    SELECT NEW.DistrictType,
           ...
           Canal,
           AttackRange
    FROM Districts_XP2 WHERE DistrictType = NEW.ReplacesDistrictType;
    ...
    SELECT NEW.DistrictType,
           YieldType,
           YieldChange
    FROM District_CitizenYieldChanges WHERE DistrictType = NEW.ReplacesDistrictType;
    INSERT INTO District_GreatPersonPoints(DistrictType, GreatPersonClassType, PointsPerTurn)
    SELECT NEW.DistrictType,
           GreatPersonClassType,
           PointsPerTurn
    FROM District_GreatPersonPoints WHERE DistrictType = NEW.ReplacesDistrictType;
    INSERT INTO District_CitizenGreatPersonPoints(DistrictType, GreatPersonClassType, PointsPerTurn)
    SELECT NEW.DistrictType,
           GreatPersonClassType,
           PointsPerTurn
    FROM District_CitizenGreatPersonPoints WHERE DistrictType = NEW.ReplacesDistrictType;
    ...
    SELECT NEW.DistrictType,
           ...
           YieldChangeAsInternationalDestination
    FROM District_TradeRouteYields WHERE DistrictType = NEW.ReplacesDistrictType;
    ...
    SELECT NEW.DistrictType,
           ...
           AppealChange,
           Description
    FROM AppealHousingChanges WHERE DistrictType = NEW.ReplacesDistrictType;

    INSERT INTO DistrictModifiers(DistrictType, ModifierId)
    SELECT NEW.DistrictType,
           ModifierId
    FROM DistrictModifiers WHERE DistrictType = NEW.ReplacesDistrictType;

    -- 继承原区域获得的相邻加成
    INSERT INTO District_Adjacencies(DistrictType, YieldChangeId)
    SELECT NEW.DistrictType,
           YieldChangeId
    FROM District_Adjacencies WHERE DistrictType = NEW.ReplacesDistrictType;

    -- 继承原区域提供的相邻加成
    INSERT INTO Adjacency_YieldChanges(ID, Description, YieldType, YieldChange, TilesRequired, AdjacentDistrict, PrereqCivic, PrereqTech, ObsoleteCivic, ObsoleteTech) SELECT
    ID||'_Amphoreus_'||NEW.DistrictType, Description, YieldType, YieldChange, TilesRequired, NEW.DistrictType, PrereqCivic, PrereqTech, ObsoleteCivic, ObsoleteTech
    FROM Adjacency_YieldChanges ay WHERE ay.AdjacentDistrict = NEW.ReplacesDistrictType;

    INSERT INTO District_Adjacencies(DistrictType, YieldChangeId)SELECT
    da.DistrictType,YieldChangeId||'_Amphoreus_'||NEW.DistrictType
    FROM District_Adjacencies da
        JOIN Adjacency_YieldChanges ay ON da.YieldChangeId = ay.ID
        WHERE ay.AdjacentDistrict = NEW.ReplacesDistrictType;

    -- 区域建成事件更稳健的写法
    -- 通过查询玩家PROPERTY获悉这是第几次建成
    INSERT OR IGNORE INTO DistrictModifiers(DistrictType, ModifierId)
    VALUES (NEW.DistrictType, 'MODFEAT_DISTRICT_PROPERTY_' || NEW.DistrictType);
    INSERT OR IGNORE INTO Modifiers(ModifierId, ModifierType)
    VALUES ('MODFEAT_DISTRICT_PROPERTY_' || NEW.DistrictType, 'MODIFIER_PLAYER_ADJUST_PROPERTY');
    INSERT OR IGNORE INTO ModifierArguments(ModifierId, Name, Value)
    VALUES ('MODFEAT_DISTRICT_PROPERTY_' || NEW.DistrictType, 'Key', 'PROPERTY_' || NEW.DistrictType),
           ('MODFEAT_DISTRICT_PROPERTY_' || NEW.DistrictType, 'Amount', 1);

END;

--======================================================================
-- 批量插入能力
INSERT OR IGNORE INTO NW_Amphoreus_Districts(DistrictType, ReplacesDistrictType)
VALUES
-- 缇宝 刻法勒广场 门径
('DISTRICT_JANUS', 'DISTRICT_HOLY_SITE'),
-- 万敌 悬锋竞技场 纷争
('DISTRICT_NIKADOR', 'DISTRICT_ENCAMPMENT'),
-- 阿格莱雅 云石天宫 浪漫
('DISTRICT_MNESTIA', 'DISTRICT_AQUEDUCT'),
-- 那刻夏 树庭 理性
('DISTRICT_CERCES', 'DISTRICT_CAMPUS'),
-- 遐蝶 龙骸古城 死亡
( 'DISTRICT_THANATOS', 'DISTRICT_ENTERTAINMENT_COMPLEX'),
-- 风堇 疗愈之庭 天空
('DISTRICT_AQUILA', 'DISTRICT_SPACEPORT'),
-- 赛飞儿 云石市集 诡计
('DISTRICT_ZAGREUS', 'DISTRICT_COMMERCIAL_HUB'),
-- 白厄 创世涡心 负世
('DISTRICT_KEPHALE', 'DISTRICT_GOVERNMENT'),
-- 海瑟音 浮影海庭 海洋
('DISTRICT_PHAGOUSA', 'DISTRICT_HARBOR'),
-- 刻律德菈 预言书库 律法
('DISTRICT_TALANTON', 'DISTRICT_DIPLOMATIC_QUARTER'),
-- 长夜月 长梦宸扉 岁月
('DISTRICT_ORONYX', 'DISTRICT_THEATER'),
-- 丹恒•腾荒 万壑岩心 大地
('DISTRICT_GEORIOS', 'DISTRICT_INDUSTRIAL_ZONE');
...
UPDATE Districts SET AllowsHolyCity=1 WHERE DistrictType = 'DISTRICT_TALANTON';

INSERT INTO DistrictModifiers(DistrictType, ModifierId) SELECT
'DISTRICT_TALANTON', ModifierId
FROM BuildingModifiers WHERE BuildingType = 'BUILDING_STONEHENGE';
...
INSERT INTO Modifiers (ModifierId, ModifierType, RunOnce, Permanent, NewOnly, OwnerRequirementSetId,
                       SubjectRequirementSetId)
VALUES ('MODIFIER_TRAIT_DISTRICT_GEORIOS_GP', 'MODIFIER_NW_AM_PVP_PLAYER_DISTRICTS_ADJUST_BASE_YIELD_CHANGE', 0, 1, 0, NULL,
...
-- Custom ModifierType
INSERT INTO Types (Type, Kind) VALUES
('MODIFIER_NW_AM_PVP_PLAYER_DISTRICTS_ADJUST_BASE_YIELD_CHANGE', 'KIND_MODIFIER');
INSERT INTO DynamicModifiers (ModifierType, CollectionType, EffectType) VALUES
('MODIFIER_NW_AM_PVP_PLAYER_DISTRICTS_ADJUST_BASE_YIELD_CHANGE', 'COLLECTION_PLAYER_DISTRICTS', 'EFFECT_ADJUST_DISTRICT_BASE_YIELD_CHANGE');
